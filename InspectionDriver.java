...
public class InspectionDriver extends HtmlUnitDriver implements TakesScreenshot {
    ...
    public InspectionDriver(boolean enableJavascript) {
        super(enableJavascript);
    }

    public InspectionDriver(Capabilities capabilities) {
        super(capabilities);
    }

    public InspectionDriver(BrowserVersion version) {
        super(version);
    ...
    private byte[] downloadCssAndImages(WebClient webClient, HtmlPage page) throws Exception {
    ...
    private String downloadCss(WebClient webClient, WebWindow window, URL resourceUrl) throws Exception {
        cssjsCache.putIfAbsent(resourceUrl.toString(), webClient.getPage(window, new WebRequest(resourceUrl))
    ...
    private byte[] downloadImage(WebClient webClient, WebWindow window, URL resourceUrl) throws Exception {
        imagesCache.putIfAbsent(resourceUrl.toString(), IOUtils.toByteArray(webClient.getPage(window, new WebRequest(resourceUrl)).getWebResponse()
                .getContentAsStream()));
    ...
    private String mapLocalUrl(HtmlPage page, URL link, String path, Map<String, String> replacementToAdd) {
